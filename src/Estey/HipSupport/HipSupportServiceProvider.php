...
class HipSupportServiceProvider extends ServiceProvider {
	...
	{
		return new HipSupport(new HipChat($this->getToken()), $this->app['config'], $this->app['cache']);
	}
	...
	protected function registerHipSupport()
	{
		...
		$this->app['hipsupport'] = $this->app->share(function($app) use ($hipsupport)
		{
			return $hipsupport;
		});	
	}	
	...
	{
		$hipsupport = $this->initHipSupport();
		$this->app['hipsupport.online'] = $this->app->share(function($app) use ($hipsupport)
		{
	...
	{
		$hipsupport = $this->initHipSupport();
		$this->app['hipsupport.offline'] = $this->app->share(function($app) use ($hipsupport)
		{
